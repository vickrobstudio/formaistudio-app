...
import { Download, ImagePlus, Instagram, LoaderCircle, Send, Sparkles, Trash2, X } from "lucide-react";
...
function PhotoAIConversation({ userId, initialMessages, input, setInput }: { userId: string | null; initialMessages: UIMessage[]; input: string; setInput: (value: string) => void }) {
  ...
  async function saveEdit() {
    ...
    try { await saveMediaToDevice(editedImage, "formai-photo-edit.png", "image/png"); }
  ...
  async function shareEditToInstagram() {
    if (!editedImage || instaState === "posting") return;
    try {
      if (signedIn) {
        setInstaState("posting");
        const shrunk = await shrinkImageDataUrl(editedImage, 1600);
        await shareToStudioInstagramFn({ data: { imageUrl: shrunk, caption: "Photo AI edit" } });
        setInstaState("posted");
      } else {
        await shareToInstagram(editedImage, "formai-photo-edit.png", "image/png");
      }
    } catch (cause) { setInstaState("idle"); setFileError(cause instanceof Error ? cause.message : "The image could not be shared."); }
  }

  // Members who connected their own Instagram in Account can post there too.
  async function shareEditToMyInstagram() {
    if (!editedImage || instaMyState === "posting") return;
    try {
      setInstaMyState("posting");
      const shrunk = await shrinkImageDataUrl(editedImage, 1600);
      await shareToMyInstagramFn({ data: { imageUrl: shrunk, caption: "Photo AI edit — made with FormAI Studio" } });
      setInstaMyState("posted");
    } catch (cause) { setInstaMyState("idle"); setFileError(cause instanceof Error ? cause.message : "The image could not be shared."); }
  }

    return <main className="min-h-screen bg-background"><FormaHeader /><div className="px-5 pt-7 md:mx-auto md:w-full md:max-w-3xl lg:max-w-4xl"><BackLink /></div><PageIntro eyebrow="Photo to AI" title="Photo AI chat" description={userId ? "Drag saved furniture into a photo, select surfaces, and render changes through AI chat." : "Upload a photo and describe the changes you want to render."} /><section className="space-y-4 px-5 pb-[calc(17rem+env(safe-area-inset-bottom))] md:mx-auto md:w-full md:max-w-3xl lg:max-w-4xl"><FurnitureLibraryPicker signedIn={signedIn} selected={furnitureReferences} onToggle={(imageUrl) => setFurnitureReferences((current) => current.includes(imageUrl) ? current.filter((item) => item !== imageUrl) : current.length < 5 ? [...current, imageUrl] : current)} />{preview && <SceneComposer sourceImage={editedImage ?? preview} signedIn={signedIn} onUseComposition={(image, instruction, texture) => { setPreview(image); setEditedImage(null); setInput(instruction); setSurfaceTexture(texture); }} />}{editedImage && <AiPlanGenerator sourceImage={editedImage} kind="space" />}<div className="space-y-4">{messages.length === 0 && <><p className="organic-divider py-8 text-center text-sm text-muted-foreground">Upload a room, position furniture, select a material area, or describe changes in chat.</p><ToolInformation sections={photoInformation} /></>}{messages.map((message) => <div key={message.id} className={`max-w-[88%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === "user" ? "ml-auto bg-foreground text-background" : "bg-secondary text-foreground"}`}>{messageImages(message).map((image, index) => <img key={`${image.url}-${index}`} src={image.url} alt="Attached room" className="mb-2 max-h-72 w-full rounded-xl object-cover" />)}{messageText(message)}</div>)}{editedImage && <div className="space-y-2"><img src={editedImage} alt="AI-edited room" className="w-full rounded-2xl border border-border" /><Button type="button" variant="outline" className="w-full" onClick={() => void saveEdit()}><Download />Save to Photos</Button><Button type="button" variant="outline" className="w-full" disabled={instaState !== "idle"} onClick={() => void shareEditToInstagram()}><Instagram />{instaState === "posted" ? "Posted to @formaistudio.app ✓" : instaState === "posting" ? "Posting to @formaistudio.app…" : signedIn ? "Post to @formaistudio.app" : "Share to Instagram"}</Button>{signedIn && instagramConnected && <Button type="button" variant="outline" className="w-full" disabled={instaMyState !== "idle"} onClick={() => void shareEditToMyInstagram()}><Instagram />{instaMyState === "posted" ? "Posted to my Instagram ✓" : instaMyState === "posting" ? "Posting to my Instagram…" : "Post to my Instagram"}</Button>}</div>}{status === "submitted" && <div className="flex items-center gap-2 text-xs text-muted-foreground"><LoaderCircle className="size-4 animate-spin" />Photo AI is thinking…</div>}{error && <p role="alert" className="text-xs text-destructive">{error.message}</p>}<div ref={endRef} /></div></section><div className="fixed inset-x-0 bottom-[max(calc(4rem+env(safe-area-inset-bottom)),var(--kb-inset,0px))] z-30 border-t border-border bg-background p-3 md:mx-auto md:max-w-3xl lg:max-w-4xl md:border-x-0 md:border-t-0"><div className="mx-auto max-w-xl">{preview && <div className="relative mb-2 w-fit"><img src={preview} alt="Selected attachment" className="h-20 w-20 rounded-xl border border-border object-cover" /><Button type="button" variant="default" size="icon" aria-label="Remove image" className="absolute -right-2 -top-2 size-7 min-h-0 rounded-full" onClick={removeImage}><X className="size-3" /></Button></div>}{fileError && <p role="alert" className="mb-2 text-xs text-destructive">{fileError}</p>}<div className="mb-2 flex items-end gap-2"><input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(event) => selectImage(event.target.files)} /><Button type="button" variant="ghost" size="icon" aria-label="Attach photo" disabled={busy || rendering} onClick={() => fileRef.current?.click()}><ImagePlus /></Button><Textarea ref={inputRef} autoFocus value={input} maxLength={2000} rows={1} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void submit(); } }} placeholder="Add, remove, relight or redesign…" className="min-h-11 resize-none rounded-2xl" /><Button size="icon" aria-label="Send message" disabled={busy || (!input.trim() && !files?.length)} onClick={() => void submit()}>{busy ? <LoaderCircle className="animate-spin" /> : <Send />}</Button><Button variant="ghost" size="icon" aria-label="Clear conversation" onClick={clear}><Trash2 /></Button></div>{preview && <Input value={locationHint} onChange={(event) => setLocationHint(event.target.value)} placeholder="Address / location for accurate surroundings (optional)" className="mb-2 h-10" />}<Button variant="studio" className="h-11 w-full" disabled={!preview || input.trim().length < 6 || rendering} onClick={() => void renderEdit()}>{rendering ? <LoaderCircle className="animate-spin" /> : <Sparkles />}{rendering ? "Rendering edit…" : "Render this edit"}</Button></div></div>{consentDialog}<ToolTabBar /></main>;
}