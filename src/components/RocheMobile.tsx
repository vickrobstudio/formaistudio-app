...
import darkLogo from "@/assets/rb-logo-dark.png.asset.json";
import whiteLogo from "@/assets/rb-logo-white.svg.asset.json";
...
export function RocheHeader({ transparent = false }: { transparent?: boolean }) {
  const [open, setOpen] = useState(false);
  const path = useRouterState({ select: (state) => state.location.pathname });
  const logo = transparent ? whiteLogo.url : darkLogo.url;
  return (
    <>
      <header className={`fixed inset-x-0 top-0 z-40 flex h-16 items-center justify-between px-5 ${transparent ? "text-primary-foreground" : "border-b border-border bg-background/95 text-foreground backdrop-blur"}`}>
        <Link to="/" aria-label="EX Roche Bobois AI Studio home"><img src={logo} alt="EX Roche Bobois AI Studio" className="h-7 w-auto max-w-44 object-contain" /></Link>
      ...
      {open && <div className="fixed inset-0 z-50 bg-foreground text-background">
        <div className="flex h-16 items-center justify-between px-5"><img src={whiteLogo.url} alt="EX Roche Bobois AI Studio" className="h-7 max-w-44 object-contain" /><Button variant="ghost" size="icon" aria-label="Close navigation" className="text-background hover:bg-background/10 hover:text-background" onClick={() => setOpen(false)}><X className="size-6" /></Button></div>
