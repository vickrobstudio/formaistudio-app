...
export function LegalPage({ title, updated, children }: { title: string; updated: string; children: ReactNode }) {
  return (
    <main className="min-h-screen bg-background text-foreground">
      <header className="flex min-h-20 items-center justify-between border-b border-border px-[max(1.25rem,env(safe-area-inset-left))] pt-[env(safe-area-inset-top)]">
        <Link to="/" aria-label="FormAI home"><FormAILogo className="h-12 w-20" /></Link>
